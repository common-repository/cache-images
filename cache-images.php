...
function mm_ci_add_pages() {
	add_management_page( __( 'Cache Remote Images', 'cache-images' ), __( 'Cache Remote Images', 'cache-images' ), 'manage_options', __FILE__,
'mm_ci_manage_page' );
}

function mm_ci_get_ajax_url($action, $url = 'action=cache_images') {
	return json_encode(
		add_query_arg(
			array(
...
<script type="text/javascript">
	function getdomains() {
		jQuery.ajax({
			...
			data: <?php echo mm_ci_get_ajax_url( 'getdomains' ); ?>,
...
<script type="text/javascript">
	function getalldomains() {
		jQuery.ajax({
			...
			type: "POST",
			//contentType: "application/json",
			data: <?php echo mm_ci_get_ajax_url( 'getalldomains' ); ?>,
			success: function(result) {
				if (result === 'null') {
					setStatusMessage('<?php _e( 'No posts with images were found.', 'cache-images' ); ?>');
					return;
...
<script type="text/javascript">
		...
		function regenerate(domain,domainmd5) {
			...
			jQuery.ajax({
				...
				type: "POST",
				//contentType: "application/json",
				data: <?php echo mm_ci_get_ajax_url( 'getlist' ); ?> + "&domain=" + domain,
				success: function(result) {
					var list = eval(result);
					var curr = 0;
					...
					function regenItem() {
						...
						jQuery.ajax({
							url: ajaxurl,
							type: "POST",
							data: <?php echo mm_ci_get_ajax_url( 'regen' ); ?>+"&url=" + list[curr].url + "&postid=" + list[curr].postid,
							success: function(result) {
								jQuery("#thumb").show();
		...
		function cacheall(domain,domainmd5) {
			...
			jQuery.ajax({
				...
				type: "POST",
				//contentType: "application/json",
				data: <?php echo mm_ci_get_ajax_url( "getalllist" ) ?> + "&domain=" + domain,
				success: function(result) {
					var list = eval(result);
					var curr = 0;
...
function cache_images_ajax() {
	...
	if ( ! is_string( $action ) || '' === trim( $action ) || ! current_user_can( 'upload_files' ) ) {
	...
	if ($action == "getlist") {
		...
		$postid_list = $wpdb->get_results( $wpdb->prepare( "SELECT DISTINCT ID FROM $wpdb->posts WHERE post_content LIKE ('%<img%') AND post_content LIKE ('%s')", '%' . $wpdb->esc_like( $domain ) . '%' ) );

		foreach ( $postid_list as $v ) {
			$postid = $v->ID;
			$post = $wpdb->get_results($wpdb->prepare("SELECT post_content FROM $wpdb->posts WHERE ID = %d", $postid));
			preg_match_all('|<img[^>]+src=([\'"])(.*?)\1.*?>|i', $post[0]->post_content, $matches);
		...
		$postid_list = $wpdb->get_results( $wpdb->prepare( "SELECT DISTINCT ID FROM $wpdb->posts WHERE post_content LIKE ('%s')", '%' . $wpdb->esc_like( $domain ) . '%' ));

		foreach ( $postid_list as $v ) {
			$postid = $v->ID;
			$post = $wpdb->get_results($wpdb->prepare("SELECT post_content FROM $wpdb->posts WHERE ID = %d", $postid));
			if ( ! preg_match_all('#(http://[^\s]+(?=\.(jpe?g|png|gif)))#i', $post[0]->post_content, $matches) ) {
				continue;
			}
		...
		foreach ($posts as $post) :
			$domains = array_merge( $domains, cache_images_find_images($post->post_content) );
		endforeach;
...
function cache_images_cache_image($url, $postid) {
	...
	if (strpos($url, 'blogspot.com') || strpos($url, 'blogger.com') || strpos($url, 'ggpht.com') || strpos($url, 'googleusercontent.com') || strpos($url, 'gstatic.com')) {
		...
		if (strpos($my_body, 'src')) {
			$spisak = null;
			preg_match_all('|<img[^>]+src=([\'"])(.*?)\1.*?>|i', $my_body, $matches);
			foreach ( $matches[2] as $url ) :
				$spisak = $url;
			endforeach;
	...
	if ( ! is_wp_error( $upload ) && preg_match_all('|<img[^>]+src=([\'"])(.*?)\1.*?>|i', $upload, $locals) ) {
		foreach ( $locals[2] as $newurl ) :
			$wpdb->query($wpdb->prepare("UPDATE $wpdb->posts SET post_content = REPLACE(post_content, '%s', '%s');", esc_url_raw( $orig_url ), esc_url_raw( $newurl )));
		endforeach;
	}
...
function cache_images_save_post($post_ID, $post) {
	...
	if ( $cache_images_settings && $cache_images_settings == 1 ) {
		...
		foreach ($domains as $domain => $num) :
			...
			if ( ! preg_match_all('|<img[^>]+src=([\'"])(.*?)\1.*?>|i', $post->post_content, $matches) ) {
				continue;
			}
			foreach ( $matches[2] as $url ) :
				if ( strstr( $url, get_option('siteurl') . '/' . get_option('upload_path') ) || !strstr( $url, $domain) || (($res) && in_multi_array($url, $res)))
			...
			$res[] = array('url' => urlencode( $url ), 'postid' => $post_ID, 'origurl' => $url);
			endforeach;
		endforeach;
